...
void *malloc(size_t size){
    ...
    void *ans = main_malloc(size);
    pthread_mutex_unlock(&malloc_mutex);
    return ans;
}

void *main_calloc(size_t number, size_t size){
    size_t *new;
    size_t s8, i;
    new = main_malloc(number * size);
    if(new){
...
void *calloc(size_t number, size_t size){
    pthread_mutex_lock(&malloc_mutex);
    void *ans = main_calloc(number, size);
    pthread_mutex_unlock(&malloc_mutex);
    return ans;
}
...
void *main_realloc(void *p, size_t size){
    size_t s;
